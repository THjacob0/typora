...
第一种情况，字符串里左方向的括号多余了 ，所以不匹配。 ![括号匹配1](https://code-thinking-1253855093.file.myqcloud.com/pics/2020080915505387.png)

第二种情况，括号没有多余，但是 括号的类型没有匹配上。 ![括号匹配2](https://code-thinking-1253855093.file.myqcloud.com/pics/20200809155107397.png)

第三种情况，字符串里右方向的括号多余了，所以不匹配。 ![括号匹配3](https://code-thinking-1253855093.file.myqcloud.com/pics/20200809155115779.png)
...
class Solution {
  ...
  public boolean isValid(String s) {
    ...
    Deque<Character> deque=new LinkedList<>();//双端队列,相当于栈
...

    {
...

      {
         ...
         if(deque.isEmpty() || deque.peek()!='(')return false;//注意在这里可能存在空的情况
...
使用一个单调队列解决该问题，保证每次在队列最前面的是最大值即可
...
使用小根堆完成只保持一个k大小的堆进行操作即可，每次将堆顶的元素排出即可
...
//初始状态：从根节点 root 开始，使用 cur 指针遍历树。
//向左遍历：将当前节点压入栈中，然后继续遍历其左子节点，直到找到最左下的节点（即没有左子节点的节点）。
//处理节点：当无法继续向左时（cur 为 NULL），开始处理栈顶元素，这个栈顶元素就是当前子树的根节点。
//访问右子树：将栈顶元素的值加入结果数组 result 中，然后转向其右子节点，继续上述过程。
class Solution {
    public List<Integer> inorderTraversal(TreeNode root) {
        List<Integer> result = new ArrayList<>();
        if (root == null){
        ...
        //!st.empty(): 当 cur 为 NULL 时，说明已经到达左子树的最左边（即当前节点没有左子节点）。此时，我们不能再继续向左移动，但我们还需要处理前面存入栈中的节点（即之前访问的节点及其右子树）。因此，!st.empty() 判断栈是否为空。如果栈不为空，说明还有尚未处理的节点（即它们的右子树还没有访问过），循环应该继续，以便处理这些节点。
           if (cur != null){
               stack.push(cur);
               cur = cur.left;
           }else{
...
使用队列来进行层序遍历，注意层序遍历用的是队列进行，而栈是将递归遍历转变为了迭代遍历因此不同
...
返回节点值自底而上的层次遍历，相对于普通层次遍历将其result数组进行反转即可
...
给定一棵二叉树，想象自己站在它的右侧，按照从顶部到底部的顺序，返回从右侧所能看到的节点值。层序遍历的时候，判断是否遍历到单层的最后面的元素，如果是，就放进result数组中，随后返回result就可以了。
